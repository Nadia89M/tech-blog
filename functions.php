<?php
/**
 * Tech Blog functions and definitions
 *
...
function tech_blog_setup() {
	/*
		...
		* If you're building a theme based on Tech Blog, use a find and replace
		* to change 'tech-blog' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'tech-blog', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'tech-blog' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'tech_blog_custom_background_args',
			array(
 ...
 */
function tech_blog_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'tech_blog_content_width', 640 );
}
add_action( 'after_setup_theme', 'tech_blog_content_width', 0 );
 ...
 */
function tech_blog_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'tech-blog' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'tech-blog' ),
...
add_action( 'widgets_init', 'tech_blog_widgets_init' );
